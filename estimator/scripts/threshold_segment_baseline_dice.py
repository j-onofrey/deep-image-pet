...
parser.add_argument('--value_range', type = int, default = 4094, help = 'The range value (max - min) of the predictions image. Default is 4096.')
parser.add_argument('--threshold', type = int, default = 1301, help = 'The threshold for bone tissue. Default = 1324 (HU +300).')
...
for subject_id, target_image in ground_truth_images.items():
    ...
    # Segment head masked target CT with threshold
    target_bone_mask = bisweb.threshold_image(target_image, bone_threshold, fixed_range + 1, replacein = True, inval = 1, replaceout = True, outval = 0, debug = debug)
