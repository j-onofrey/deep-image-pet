...
class AdversarialBinaryCrossEntropy(loss.Loss):

    def __init__(self):
        super().__init__()
        self.name = 'AdversarialBinaryCrossEntropy'
        return None
    ...
    def generator_loss(self, gan_model, add_summaries = False):
        with tf.variable_scope('generator_loss'):
            ...
            if add_summaries:
                tf.summary.scalar('generator_binary_cross_entropy_loss', g_loss)
    ...
    def discriminator_loss(self, gan_model, add_summaries = False):
        with tf.variable_scope('discriminator_loss'):
            ...
            tf.summary.histogram('d_real', tf.nn.sigmoid(d_real), family = 'discriminator_outputs')
            tf.summary.histogram('d_fake', tf.nn.sigmoid(d_fake), family = 'discriminator_outputs')
            ...
            if add_summaries:
                tf.summary.scalar('discriminator_binary_cross_entropy_loss', d_loss)

            return d_loss


def New():
    return AdversarialBinaryCrossEntropy()
