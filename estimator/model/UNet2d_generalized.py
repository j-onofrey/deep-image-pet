...
def create_model_fn(input_data,model_params,params=None,name='U-Net'):
    ...
    with tf.variable_scope(name) as scope:
        ...
        pad_size = ((filter_size-1)//2+(dilation_rate-1))*num_convs_per_layer
        ...
        for clayer in range(1,num_conv_layers+1):
            ...
            with tf.variable_scope("conv_"+cname) as scope:
                ...
                conv = tf.layers.conv2d(inputs=padding,
                ...
                if do_norm:
                    ...
                    conv = normalization_function(conv, training = (mode == tf.estimator.ModeKeys.TRAIN), name = 'encode_norm_1')
                ...
                for i in range(1,num_convs_per_layer):
                    ...
                    if do_norm:
                        tf.logging.debug('Adding normalization')
                        conv = normalization_function(conv, training = (mode == tf.estimator.ModeKeys.TRAIN), name = 'encode_norm_' + str(i+1))
        ...
        with tf.variable_scope("conv_"+cname) as scope:
            ...
            relu = tf.nn.relu(conv,name='relu_'+cname+'_1')
            tf.logging.debug('feature shape: '+str(relu.get_shape()))
            tf.logging.debug('Using dropout_rate=%f, training=%s' % (dropout_rate, (mode == tf.estimator.ModeKeys.TRAIN)))
            dropout = tf.layers.dropout(relu, rate=dropout_rate, name='dropout_'+cname+'_1', training=(mode == tf.estimator.ModeKeys.TRAIN))

            for i in range(1,num_convs_per_layer):
                conv_name = str(i+1)
                conv = tf.layers.conv2d(inputs=dropout,
                                filters=num_out_channels,
                                kernel_size=filter_size,
                                dilation_rate=dilation_rate,
                                strides=1,
                ...
                if do_norm:
                    tf.logging.debug('Adding normalization')
                    conv = normalization_function(conv, training = (mode == tf.estimator.ModeKeys.TRAIN), name = 'mid_norm_' + str(i+1))

                relu = tf.nn.relu(conv,name='relu_'+cname+'_'+conv_name)
                tf.logging.debug('feature shape: '+str(relu.get_shape()))
                tf.logging.debug('Using dropout_rate=%f, training=%s' % (dropout_rate, (mode == tf.estimator.ModeKeys.TRAIN)))
                dropout = tf.layers.dropout(relu, rate=dropout_rate, name='dropout_'+cname+'_'+conv_name, training=(mode == tf.estimator.ModeKeys.TRAIN))
        ...
        while (dlayer>0):
            ...
            with tf.variable_scope("conv_transpose_"+dname):
                ...
                if do_norm:
                    tf.logging.debug('Adding normalization')
                    conv = normalization_function(conv, training = (mode == tf.estimator.ModeKeys.TRAIN), name = 'decode_norm_1')
                ...
                for i in range(1,num_convs_per_layer):
                    ...
                    if do_norm:
                        tf.logging.debug('Adding normalization')
                        conv = normalization_function(conv, training = (mode == tf.estimator.ModeKeys.TRAIN), name = 'decode_norm_' + str(i+1))
...
class UNet2d_generalized(base_model.UNet2d):
    ...
    def __init__(self):
        super().__init__()
        self.model_name = 'U-Net 2D (generalized)'
        self.dim = 2

        self.model_params.update(UNet2d_generalized.default_model_params)
        self.param_description.update(UNet2d_generalized.default_param_description)
    ...
    def add_command_line_parameters(self,parser,training):
        ...
        return bis_util.add_module_args(model_group, UNet2d_generalized.default_model_params, UNet2d_generalized.default_param_description)

    def set_parameters(self,args,training=False,saved_params=[]):
        super().set_parameters(args,training)
        bis_util.set_multiple_parameters(self.model_params, UNet2d_generalized.default_model_params, args, new_dict = saved_params)
