...
def create_model_fn(input_data, model_params, params=None, name='U-Net'):
    ...
    with tf.variable_scope(name) as scope:
        ...
        if in_shape[-1] <4:#ls
            tf.logging.error('input data at least should have 4 channels: image and 3 location coords')
            sys.exit(1)
        image_data=input_data[:,:,:,:,0:-3]
        loc_data=input_data[:,:,:,:,-3:]

        img_shape = image_data.get_shape()
        if len(img_shape) - 1 < 4:
            image_data = tf.expand_dims(image_data, axis=-1, name="expand_input_channel")

        size_patch = tf.to_float(in_shape[1]*in_shape[2]*in_shape[3])
        loc_data = tf.reduce_sum(loc_data,[1,2,3])
        loc_data = loc_data/size_patch #calculate the mean locations for each sample, out put should have size BatchSize*3 (X,Y,Z)
        ...
        while (dlayer > 0):
            ...
            with tf.variable_scope("conv_transpose_" + dname):
                ...
                for i in range(1, num_convs_per_layer):
                    ...
                    if (dlayer==1):#ls:only apply scaling and bias at the last layer
                        dense_scalar = tf.layers.dense(inputs=loc_data,
                                                units=64,
                                                activation=tf.nn.relu,
                                                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2),
                                                name='dense_FILM_scalar1') #adde by ls
                        tf.logging.debug('Dense FILM scalar shape: %s',str(dense_scalar.get_shape()))
                        dense_scalar = tf.layers.dense(inputs=dense_scalar,
                                                units=128,
                                                activation=tf.nn.relu,
                                                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2),
                                                name='dense_FILM_scalar2') #adde by ls
                        tf.logging.debug('Dense FILM scalar shape: %s',str(dense_scalar.get_shape()))
                        dense_scalar = tf.layers.dense(inputs=dense_scalar,
                                                units=256,
                                                activation=tf.nn.relu,
                                                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2),
                                                name='dense_FILM_scalar3') #adde by ls
                        tf.logging.debug('Dense FILM scalar shape: %s',str(dense_scalar.get_shape()))
                        dense_scalar = tf.layers.dense(inputs=dense_scalar,
                                                units=num_out_channels,
                                                activation=tf.nn.relu,
                                                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2),
                                                name='dense_FILM_scalar4') #adde by ls
                        tf.logging.debug('Dense FILM scalar shape: %s',str(dense_scalar.get_shape()))
                        dense_bias = tf.layers.dense(inputs=loc_data,
                                                units=64,
                                                activation=tf.nn.relu,
                                                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2),
                                                name='dense_FILM_bias1') #adde by ls
                        tf.logging.debug('Dense FILM bias shape: %s',str(dense_bias.get_shape()))
                        dense_bias = tf.layers.dense(inputs=dense_bias,
                                                units=128,
                                                activation=tf.nn.relu,
                                                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2),
                                                name='dense_FILM_bias2') #adde by ls
                        tf.logging.debug('Dense FILM bias shape: %s',str(dense_bias.get_shape()))
                        dense_bias = tf.layers.dense(inputs=dense_bias,
                                                units=256,
                                                activation=tf.nn.relu,
                                                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2),
                                                name='dense_FILM_bias3') #adde by ls
                        tf.logging.debug('Dense FILM bias shape: %s',str(dense_bias.get_shape()))
                        dense_bias = tf.layers.dense(inputs=dense_bias,
                                                units=num_out_channels,
                                                activation=tf.nn.relu,
                                                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2),
                                                name='dense_FILM_bias4') #adde by ls
                        tf.logging.debug('Dense FILM bias shape: %s',str(dense_bias.get_shape()))
                        #conv apply scalar and bias from dense
                        dense_scalar = tf.expand_dims(dense_scalar, 1)#?*64->?*1*64
                        dense_scalar = tf.expand_dims(dense_scalar, 1)
                        dense_scalar = tf.expand_dims(dense_scalar, 1)#?*1*1*1*64
                        dense_bias = tf.expand_dims(dense_bias, 1)#?*64->?*1*64
                        dense_bias = tf.expand_dims(dense_bias, 1)
                        dense_bias = tf.expand_dims(dense_bias, 1)#?*1*1*1*64
                        conv = tf.multiply(conv,dense_scalar,name='dense_FILM_scaling')
                        conv = tf.add(conv,dense_bias,name='dense_FILM_addbias')
...
class UNet3d_generalized_FILM(base_model.UNet3d):
    ...
    def __init__(self):
        super().__init__()
        self.model_name = 'U-Net 3D (generalized FILM)'
        self.dim = 3

        self.model_params.update(UNet3d_generalized_FILM.default_model_params)
        self.param_description.update(UNet3d_generalized_FILM.default_param_description)
    ...
    def add_command_line_parameters(self, parser, training):
        ...
        return bis_util.add_module_args(model_group, UNet3d_generalized_FILM.default_model_params,
                                        UNet3d_generalized_FILM.default_param_description)

    def set_parameters(self, args, training=False, saved_params=[]):
        super().set_parameters(args, training)
        bis_util.set_multiple_parameters(self.model_params, UNet3d_generalized_FILM.default_model_params, args,
...
def New():
    return UNet3d_generalized_FILM()
