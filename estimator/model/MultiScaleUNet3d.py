    ...
    with tf.variable_scope(name) as model_scope:
        ...
        for level_i in reversed(range(model_params['num_levels'])):
            ...
            with tf.variable_scope('decode_level_%i' % level_i) as level_scope:
                ...
                current_layer = tf.layers.conv3d_transpose(
                    inputs = current_layer,
...
class MultiScaleUNet3d(base_model.Model):
    """Model subclass that implements a Multi Scale U-Net model in 3D.
    """
    ...
    def __init__(self):
        """Multi Scale U-Net 3D constructor method.
        Initializes Multi Scale U-Net specific model parameters.
        """
        super().__init__()

        self.model_name = 'Multi Scale U-Net 3D'
        self.dim = 3

        self.model_params.update(MultiScaleUNet3d.default_model_params)
        self.param_description.update(MultiScaleUNet3d.default_param_description)
    ...
    def add_command_line_parameters(self,parser,training):
        ...
        return bis_util.add_module_args(model_group, MultiScaleUNet3d.default_model_params, MultiScaleUNet3d.default_param_description)

    def set_parameters(self, args, training = False, saved_params = []):
        super().set_parameters(args, training)
        bis_util.set_multiple_parameters(self.model_params, MultiScaleUNet3d.default_model_params, args, new_dict = saved_params)
