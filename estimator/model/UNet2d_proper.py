    ...
    with tf.variable_scope(name) as scope:
        ...
        for clayer in range(1,num_conv_layers+1):
            ...
            with tf.variable_scope("conv_"+cname) as scope:
                ...
                relu = tf.layers.conv2d(inputs=clayer_inputs[clayer-1], 
        ...
        while (dlayer>0):
            ...
            with tf.variable_scope("conv_transpose_"+dname):
                ...
                if bridge_mode == 'concat':
                    clayer_shape = clayer_out.get_shape()
                    upconv_shape = upconv.get_shape()
                    crop_offsets  = [0, (clayer_shape[1].value-upconv_shape[1].value)//2, (clayer_shape[2].value-upconv_shape[2].value)//2, 0]
                    crop_size = [-1, upconv_shape[1].value, upconv_shape[2].value, -1]
                    clayer_out_crop = tf.slice(clayer_out, crop_offsets, crop_size)
                    tf.logging.debug('crop shape: '+str(clayer_out_crop.get_shape()))

                    feature_concat = tf.concat([clayer_out_crop, upconv], axis=dindex, name='concat_'+dname)
                    tf.logging.debug('concat shape: '+str(feature_concat.get_shape()))
                else:
                    feature_concat = tf.identity(upconv, name='identity_'+dname)

                relu = tf.layers.conv2d(inputs=feature_concat, 
                                filters=num_out_channels, 
                                kernel_size=filter_size, 
                                strides=1, 
                                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2),
...
class UNet2d_proper(base_model.Model):
    ...
    def __init__(self):
        ...
        self.model_params.update(UNet2d_proper.default_model_params)
        self.param_description.update(UNet2d_proper.default_param_description)
    ...
    def add_command_line_parameters(self,parser,training):
        ...
        return bis_util.add_module_args(model_group, UNet2d_proper.default_model_params, UNet2d_proper.default_param_description)



    def set_parameters(self,args,training=False,saved_params=[]):
        super().set_parameters(args,training)
        bis_util.set_multiple_parameters(self.model_params, UNet2d_proper.default_model_params, args, new_dict = saved_params)

    # Class level access to the model function
    def create_model(self,input_data):
        model_output = create_model_fn(input_data,model_params=self.model_params)
        
        # Calculate the output cropping params (offset only since the size is implicitly specified by the output)
        input_shape = input_data.get_shape()
        output_shape = model_output.get_shape()
        self.output_crop_offset = [0, (input_shape[1].value-output_shape[1].value)//2, (input_shape[2].value-output_shape[2].value)//2, 0]
        self.output_crop_size = [-1,output_shape[1].value,output_shape[2].value,-1]
        return model_output


def New():
    return UNet2d_proper()
