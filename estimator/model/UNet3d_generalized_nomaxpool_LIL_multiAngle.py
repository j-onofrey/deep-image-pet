    ...
    with tf.variable_scope(name) as scope:
        ...
        for clayer in range(1, num_conv_layers + 1):
            ...
            with tf.variable_scope("conv_" + cname) as scope:
                ...
                pool = tf.layers.conv3d(inputs=relu,
                                filters=num_out_channels,
                                kernel_size=2,
                                strides=2,
                                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2),
                                activation=tf.nn.relu,
                                name='convolution_'+cname+'_pooling')
...
class UNet3d_generalized_nomaxpool_LIL_multiAngle(base_model.UNet3d):
    ...
    default_param_description = {
		...
		'weight_LIL': 'Weight for the line integral loss, default = 0.0',
        'num_angle': 'Number of different angles'
    }

    def __init__(self):
        super().__init__()
        self.model_name = 'U-Net 3D (generalized no Pooling LIL)'
        self.dim = 3

        self.model_params.update(UNet3d_generalized_nomaxpool_LIL_multiAngle.default_model_params)
        self.param_description.update(UNet3d_generalized_nomaxpool_LIL_multiAngle.default_param_description)
    ...
    def add_command_line_parameters(self, parser, training):
        ...
        return bis_util.add_module_args(model_group, UNet3d_generalized_nomaxpool_LIL_multiAngle.default_model_params,
                                        UNet3d_generalized_nomaxpool_LIL_multiAngle.default_param_description)

    def set_parameters(self, args, training=False, saved_params=[]):
        super().set_parameters(args, training)
        bis_util.set_multiple_parameters(self.model_params, UNet3d_generalized_nomaxpool_LIL_multiAngle.default_model_params, args,
    ...
    def get_loss_function(self, output_layer, labels):
        ...
        if True:
            output_layer_squeeze = tf.squeeze(output_layer,-1)
            labels_squeeze = tf.squeeze(labels,-1)
            #LI_loss = tf.Variable(0.0,trainable=False)
            LI_loss=0.0
            num_angle = self.model_params['num_angle']
            angle_inc = 180.0/num_angle
            for rot_ind in range(0,num_angle):
                rot_angle = math.radians(rot_ind*angle_inc)
                output_rotated = tf.contrib.image.rotate(output_layer_squeeze, rot_angle, interpolation = 'BILINEAR')
                labels_rotated = tf.contrib.image.rotate(labels_squeeze, rot_angle, interpolation='BILINEAR')
                output_proj = tf.reduce_sum(output_rotated,axis=1)
                label_proj = tf.reduce_sum(labels_rotated, axis=1)
                error_proj = tf.reduce_mean(tf.square(tf.subtract(output_proj, label_proj)))
                LI_loss = tf.add(LI_loss, error_proj)
            LI_loss = tf.divide(LI_loss, num_angle)
            weighted_LIL = self.model_params['weight_LIL']*LI_loss
            tf.summary.scalar('weighted_LIL', weighted_LIL, family='losses')
        ...
        if self.loss_function is not None:
            ...
            return reconstruction_loss + weighted_GDL + weighted_LIL
...
def New():
    return UNet3d_generalized_nomaxpool_LIL_multiAngle()
